...
function InitializeDisableButtons(selectors)
{
	if(selectors == null)
	{
	...
	buttonNodeList.forEach(buttonNode => {
		...
		buttonNode.addEventListener('click', function(event) {
			DisableButtonsOnPage(selectors, event);
		});
	});
	...
	if(buttonNodeList.length > 0)
	{
		...
		img.src = DISABLE_BUTTONS_WAIT_IMAGE_URL;
	}
}
...
function DisableButtonsOnPage(selectors, event)
{
	// Get all buttons
	...
	buttonNodeList.forEach(buttonNode => {
		...
		if (event.target == buttonNode)
		{
			...
			buttonNode.style.backgroundImage = "url('" + DISABLE_BUTTONS_WAIT_IMAGE_URL + "')";
